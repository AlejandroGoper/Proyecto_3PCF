...
void iso3PCF::XXX(int i,int j,Puntos *p, unsigned int ***HHH){
    ...
    l1 = euclidean_dist3D<float>(p[i].x - p[j].x, p[i].y - p[j].y, p[i].z - p[j].z);
    if (l1 < d_max)
    {
        for (k = j+1; k < num_puntos; k++)
        {
            l2 = euclidean_dist3D<float>(p[i].x - p[k].x, p[i].y - p[k].y, p[i].z - p[k].z);
            if (l2 < d_max)
            {
                l3 =  euclidean_dist3D<float>(p[j].x - p[k].x, p[j].y - p[k].y, p[j].z - p[k].z);
                if (l3 < d_max)
                {
                    a = (int)(l1*ds);
                    b = (int)(l2*ds);
